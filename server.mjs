...
export function render(data,slug,base,download){
 ...
 <footer>点赞 ${Number(p.like_count)||0} · 评论 ${Number(p.reply_count)||0}</footer><nav><a href="${p.source_type==='note'?`huideng://shared-note/${slug}`:`huideng://forum/post/${encodeURIComponent(p.id)}?slug=${slug}`}">打开文殊计数器</a>${safeUrl(download)?`<a href="${escape(safeUrl(download))}">下载 App</a>`:'<button type="button" disabled title="安装包下载地址尚未提供">下载 App（暂未开放）</button>'}</nav><h2>相关内容</h2>${(data.related??[]).filter(x=>/^[a-f0-9]{64}$/.test(x.slug)).map(x=>`<p><a href="/p/${x.slug}">${escape(getPostDisplayTitle(x))}</a></p>`).join('')}</main></body></html>`;
}
