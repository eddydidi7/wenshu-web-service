...
export function render(data,slug,base,download){
 ...
 <meta name="description" content="${escape(summary)}"><meta property="og:title" content="${escape(title)}"><meta property="og:description" content="${escape(summary)}"><meta property="og:site_name" content="文殊红书"><meta property="og:type" content="article"><meta property="og:url" content="${escape(base)}/p/${slug}">${cover?`<meta property="og:image" content="${escape(cover)}">`:''}${p.access_level==='link_only'?'<meta name="robots" content="noindex,nofollow">':''}
 <style>body{background:#161513;color:#eee8df;font:18px/1.8 system-ui;margin:0}main{max-width:760px;margin:auto;padding:24px}h1{font-size:30px;line-height:1.4}img{max-width:100%;height:auto;border-radius:12px}a{color:#d9b879}header,footer{color:#aaa}article{overflow-wrap:anywhere;margin:24px 0}nav{display:flex;gap:16px;flex-wrap:wrap}nav a{padding:8px 14px;border:1px solid #706049;border-radius:8px;text-decoration:none}.avatar{width:36px;height:36px;border-radius:50%;vertical-align:middle} .tags{color:#d9b879}</style></head><body><main><header>文殊红书</header><h1>${escape(title)}</h1><header>${safeUrl(p.author_avatar)?`<img class="avatar" alt="作者头像" src="${escape(safeUrl(p.author_avatar))}">`:''} ${escape(p.author_name)} · ${escape(p.created_at)}</header><div class="tags">${(p.tags??[]).map(t=>'#'+escape(t)).join(' ')}</div><article>${richText(p.rich_body,p.body)}${(p.image_urls??[]).map(safeUrl).filter(Boolean).map(u=>`<p><img alt="文章图片" loading="lazy" src="${escape(u)}"></p>`).join('')}</article>
 ...
 <footer>点赞 ${Number(p.like_count)||0} · 评论 ${Number(p.reply_count)||0}</footer><nav><a href="huideng://forum/post/${encodeURIComponent(p.id)}?slug=${slug}">打开文殊计数器</a>${safeUrl(download)?`<a href="${escape(safeUrl(download))}">下载文殊计数器</a>`:'<span>下载文殊计数器（入口尚未配置）</span>'}</nav><h2>相关内容</h2>${(data.related??[]).filter(x=>/^[a-f0-9]{64}$/.test(x.slug)).map(x=>`<p><a href="/p/${x.slug}">${escape(x.title)}</a></p>`).join('')}</main></body></html>`;
}
export function handler(env=process.env,fetcher=fetch){return async(req,res)=>{
 ...
 const match=/^\/p\/([a-f0-9]{64})$/.exec(path);if(!match){reply(404,'页面不存在');return;}
 ...
 try{
  ...
  const response=await fetcher(`${upstream}/functions/v1/shared-page`,{method:'POST',headers,body:JSON.stringify({slug:match[1]}),signal:AbortSignal.timeout(15000)});
  ...
  res.end(req.method==='HEAD'?'':render(data,match[1],base,env.DOWNLOAD_URL));
